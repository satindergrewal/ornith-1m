#!/usr/bin/env python3
"""tc_holdout_gate.py — official ThinkingCap before/after claim, ours measured.

Holdout problems seed 20260905 (12 problems, saved unanswered at regen time,
FIRST touched here). Two arms:
  base   : the ablit engine as validated
  capped : engine + trained LoRA folded into the loaded bf16 weights
           (W_eff = W + (alpha/r) B@A — the exact 05-merge semantics, applied
           in memory; the checkpoint copy merge is the separate stopped-before
           step)
Both arms: 3 samples per problem, temp 0.8, budget 160, official chat template,
SAME sample seed (paired). Metrics per arm: per-kind correctness (strict +
first-answer), mean completion tokens (all + correct-only), hit-cap count.
GATE (coordinator): accuracy preserved within noise of base AND completion
length measurably bounded.
"""
...
from tc_batch_gen import build_prompt, make_problems, parse_answer, first_answer
...
def run_arm(arm, lora_path=None):
    ...
    for r in seqs:
        ...
        rows.append({"arm": arm, "pid": r["pid"], "kind": r["kind"],
                     ...
                     "completion": text})
    out = f"/wd/data/holdout_{arm}.jsonl"
    ...
    def summarize(rs):
        ...
        return {"samples": len(rs),
                ...
                "mean_tokens_all": round(sum(r["n_new"] for r in rs) / max(len(rs), 1), 1),
    ...
    rep = {"arm": arm, "lora": lora_path, "seed": HOLDOUT_SEED,
           ...
           "per_kind": {str(k): summarize([r for r in rows if r["kind"] == k])
                        for k in (0, 1, 2)}}
    json.dump(rep, open(f"/wd/data/holdout_{arm}_report.json", "w"), indent=1)
    print(f"[gate/{arm}] REPORT: {json.dumps(rep['overall'])}", flush=True)
    print(f"[gate/{arm}] per-kind: {json.dumps(rep['per_kind'])}", flush=True)
    for r in rows[:6]:
        print(f"   pid{r['pid']} k{r['kind']} ok={r['correct']} n={r['n_new']}: "
              f"{r['completion'][:70]!r}", flush=True)
    return rep


def compare():
    b = json.load(open("/wd/data/holdout_base_report.json"))
    c = json.load(open("/wd/data/holdout_capped_report.json"))
